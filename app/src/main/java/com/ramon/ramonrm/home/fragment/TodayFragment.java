...
public class TodayFragment extends Fragment implements FlagmentDataRefreshInterface {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        //不同的Activity对应不同的布局
        View view = inflater.inflate(R.layout.fragment_todaypage, container, false);
        if (isLoad) {
            loadChart(hashMap);
        }
        return view;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        chartChanPin = (ColumnChartView) mActivity.findViewById(R.id.fragment_todaypage_chartjsbj);
        if (!isLoad) {
            isLoad = true;
    ...
    private void loadChart(HashMap<String, String>[] hashData) {
        ...
        for (int i = 0; i < numColumns; i++) {
            ...
            chartPoint.XValue = hashData[i].get("dayTime");
            chartPoint.YValue = Integer.parseInt(hashData[i].get("alarmNum"));
